...
def get_badge(text, value, color=DEFAULT_COLOR, style=DEFAULT_STYLE):
    ...
    return template.replace(
            ...
            '{{ text_width }}',
            str(len(text) * 9 * STYLES[style])
        ).replace(
            '{{ value_width }}',
            str(len(value) * 9 * STYLES[style])
        ).replace(
            '{{ total_width }}',
            str(len(text) * 9 * STYLES[style] + len(value) * 9 * STYLES[style])
        ).replace(
            '{{ value_text_width }}',
            str(len(value) * 50 * STYLES[style])
        ).replace(
            '{{ user_text_width }}',
            str(len(text) * 50 * STYLES[style])
        ).replace(
            '{{ user_text_margin }}',
            str((len(text) * 90 * STYLES[style]) / 2)
        ).replace(
            '{{ value_text_margin }}',
            str(len(text) * 90 * STYLES[style] + len(value) * 90 * STYLES[style] / 2)
        )
...
def parse_args(argv=None):
    ...
    parser.add_argument('-git', dest='git', action='store_true',
            help='Build badge for git tag.')
    parser.add_argument('-cov', dest='cov', action='store_true',
            help='Build badge for coverage.')
...
def main(argv=None):
    ...
    # Validation
    if not args.git and not args.cov and not args.text and not args.value:
